...
trait BuilderBeingTested[Params, BFn, R, RFn, B <: Builder[R, RFn, B], E <: Engine[Params, BFn, R, RFn]] {
  ...
  def because(seed: Int) = builder = becauseImpl(seed)
  def code(seed: Int) = update(_.codeHolder(resultCodeHolder(seed)))
  ...
  protected def becauseImpl(seed: Int): B
  protected def scenarioObject(params: Params): Scenario[Params, BFn, R, RFn]
  protected def s(seed: Int) = scenarioObject(params(seed))
  protected def resultCodeHolder(seed: Int): CodeHolder[RFn]
  protected def becauseCodeHolder(seed: Int) = CodeHolder[BFn](becauseBfn(seed), s"because$seed")
...
trait StringStringTest {
  ...
  def params(seed: Int): String = s"A$seed"
...
trait StringStringStringTest {
  ...
  def becauseBfn(seed: Int) = (p1: String, p2: String) => (p1 == params(seed)._1 && p2 == params(seed)._2)
}
trait StringStringStringStringTest {
  ...
  def result(seed: Int) = seed.toString
  def becauseBfn(seed: Int) = (p1: String, p2: String, p3: String) => (p1 == params(seed)._1 && p2 == params(seed)._2) && p3 == params(seed)._3
}
