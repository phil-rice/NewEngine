...
object Builder1 {
  ...
  //  def matchWithImpl[P: c.WeakTypeTag, R: c.WeakTypeTag](c: Context)(pf: c.Expr[PartialFunction[P, R]]): c.Expr[Builder1[P, R, FullR]] = {
  //    import c.universe._
  //    val thisObject: c.Expr[Builder1[P, R, FullR]] = (c.Expr[Builder1[P, R, FullR]](c.prefix.tree))
  //    reify {
...
case class Builder1[P, R, FullR](nodes: List[BuilderNode[R, (P) => R]] = List(new EngineDescription[R, (P) => R]),
  ...
  def copyNodes(nodes: List[BuilderNode[R, (P) => R]]) = wrap(new Builder1[P, R, FullR](nodes, buildExceptions))
  def build: Engine1[P, R] = BuildEngine.build1(this)
  def copyWithNewExceptions(buildExceptions: Map[BuilderNode[R, (P) => R], List[Exception]]) = new Builder1[P, R, FullR](nodes, buildExceptions)
}
